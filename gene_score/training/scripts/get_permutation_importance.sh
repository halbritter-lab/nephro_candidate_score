...
file_path="/data/gpfs-1/users/rankn_c/work/halbritter/nephro_candidate_score/gene_score/training/scripts/get_permutation_importance.py"

# python "$file_path" 2>&1 | tee slurm_out/verbose_out_$SLURM_JOB_ID.txt

python "$file_path" --ID "$training_ID" 2>&1 | tee gene_score/training/
slurm_out/verbose_out_$SLURM_JOB_ID.txt
