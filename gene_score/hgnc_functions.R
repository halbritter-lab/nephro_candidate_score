...
hgnc_id_from_symbol_grouped <- function(input_tibble, request_max = 150) {
  ...
  cat("getting hgnc id from symbol... \n")
  pb <- progress_bar$new(total = groups_number)
  ...
  input_tibble_request <- input_tibble %>%
    ...
    mutate(response = {
      pb$tick()
      hgnc_id_from_symbol(value)$hgnc_id}) %>%
    ungroup()
  
  cat("getting hgnc id from previous symbol... \n")
  no_na_values <- input_tibble_request %>% filter(is.na(response)) %>% nrow() # needed for progress bar
  pb1 <- progress_bar$new(total = no_na_values)
  ...
  input_tibble_request_repair <- input_tibble_request %>%
    ...
    mutate(response = {
      pb1$tick()
      hgnc_id_from_prevsymbol(value)}) 
  
  cat("getting hgnc id from alias symbol... \n")
  pb2 <- progress_bar$new(total = no_na_values)
  
  input_tibble_request_repair <- input_tibble_request_repair %>%
    mutate(response = {
      pb2$tick()
      case_when(!is.na(response) ~ response, is.na(response) ~ hgnc_id_from_aliassymbol(value))})
