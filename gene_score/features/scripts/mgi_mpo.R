...
download.file(mgi_mpo_url, 
              destfile = paste0("raw/mgi_mpo_", config_vars$creation_date_gs, ".json"))

# load MPO data
mp <- fromJSON(paste0("raw/mgi_mpo_", config_vars$creation_date_gs, ".json"))
...
download.file(url = download_url, 
              destfile = paste0("raw/HMD_HumanPhenotype_", config_vars$creation_date_gs, ".rpt"))
hmd_hp <- read.table(paste0("raw/HMD_HumanPhenotype_", config_vars$creation_date_gs, ".rpt"), sep="\t", header=FALSE) %>% dplyr::select(-V6)
...
download.file(url = download_url, 
              destfile = paste0("raw/MGI_PhenoGenoMP_", config_vars$creation_date_gs, ".rpt"))

mgi_pg_mp <- read.delim(paste0("raw/MGI_PhenoGenoMP_", config_vars$creation_date_gs,  ".rpt"), header=FALSE)
...
# write results
write.csv(mgi_pg_mp, 
          paste0("results/mgi_human_genes_associated_MP_0005367_" , config_vars$creation_date_gs, ".csv"), 
          row.names = FALSE)

gzip(paste0("results/mgi_human_genes_associated_MP_0005367_" , config_vars$creation_date_gs, ".csv"),
     overwrite = TRUE)
